...
function* getTokenDetail(action: Action) {
  console.log("here is getTokenDetail");
  try {
    const request_url = REQUEST_HOST + "/" + TOKEN_GET_METHOD;
    const request_scope = request_url + "/" + API_VERSION;

    let requestData = new FormData();
    let requestData_sign = {
      token_id: action.payload.tokenId,
    };
    requestData.append("token_id", action.payload.tokenId);
    ...
    if (data.SUCCESS === false && data.MESSAGE !== "") {
      yield put(nft.getTokenFailure(data));
    } else {
      yield put(nft.getTokenSuccess(data.DATA));
    }
  } catch (e: any) {
    yield put(nft.getTokenFailure({ error: { ...e.response.data } }));
  }
}

export function* watchTokenOperations() {
  console.log("here is listing saga");
  yield all([
    // @ts-ignore
    takeEvery(NFT.DETAIL_REQUEST, getTokenDetail),
  ]);
}

export default watchTokenOperations;
